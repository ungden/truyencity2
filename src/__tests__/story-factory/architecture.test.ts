...
describe('Story Factory architecture boundary', () => {
  ...
  test('a new story validates its opening plan before paying for a cover', () => {
    ...
    expect(read('src/services/story-factory/prompts.ts')).toContain('Không sửa một deltaCheck bằng cách chép before/delta/after');
    expect(read('src/services/story-factory/pipeline.ts')).toContain("resource.unit.includes('_')");
  });
