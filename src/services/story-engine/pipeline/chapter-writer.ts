...
QUY TẮC:
1. Pacing theo "ức chế → bùng nổ" — mỗi chương ít nhất 1 khoảnh khắc thỏa mãn hoặc sảng khoái (kể cả truyện điền viên, kinh doanh cũng cần thu hoạch, chốt deal hoặc sự bình yên hài hước). LƯU Ý: "ức chế → bùng nổ" KHÔNG có nghĩa là dập MC liên tục — phải có chương đệm giữa các đợt ức chế.
...
6. CHỐNG TỰ NGƯỢC + PROACTIVE NARRATIVE (TƯ DUY THEO GIAI ĐOẠN, KHÔNG ĐẾM CHƯƠNG):
   ...
   - QUYẾT ĐỊNH NHỊP CHƯƠNG NÀY:
     ...
     • Nếu giai đoạn ngược đã kéo dài 4+ chương mà chưa resolve → chương này PHẢI bắt đầu resolution (MC thắng, thua nhưng sống, hoặc tìm được lối thoát). CẤM kéo dài lê thê 6+ chương.
...
LƯU Ý MOOD CHƯƠNG (CRITIC LITE MODE):
- Nếu chương có mood "breathing" / "comedic_break" / "calm_before_storm" / "comfort": ÁP DỤNG STANDARD NHẸ.
  • CẤM downgrade overallScore vì thiếu plot tension/cliffhanger nguy hiểm — đây là bản chất breathing chapter.
  • CẤM yêu cầu rewrite vì "thiếu conflict" hoặc "thiếu dopamine moment" — small wins / casual competence / observation đời thường là valid output.
  • Vẫn check: continuity, repetition, character voice, word count. Bỏ qua "missing stakes/hooks/dopamine intensity" cho breathing.
  • Tối thiểu pass: overallScore ≥5 đủ để approve (thay vì 6 như chuẩn).
