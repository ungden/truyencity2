...
export const FLAGSHIP_V3_PROMPT_VERSION = 'flagship-v3.16-grounded-rewrite-length';
...
Viết từng scene theo đúng sceneId và thứ tự, chỉ đi vào nội tâm của povCharacterId. Mỗi scene phải thành một đơn vị kịch tính hoàn chỉnh trước khi chuyển cảnh; dừng khi hiệu ứng hookIntent cuối đã hình thành, không thêm cảnh ngoài plan hoặc lời tổng kết để kéo chữ.
...
export function buildV3WriterPrompt(input: {
  ...
  const naturalTarget = Math.min(
    V3_CHAPTER_LENGTH_POLICY.softMaxWords,
    Math.max(V3_CHAPTER_LENGTH_POLICY.softMinWords, input.targetWordCount),
  );
  return `Viết chương ${input.chapterNumber}. Bản hoàn chỉnh phải nằm trong hard range ${V3_CHAPTER_LENGTH_POLICY.hardMinWords}-${V3_CHAPTER_LENGTH_POLICY.hardMaxWords} từ; hãy nhắm khoảng ${naturalTarget} từ để không vô tình rơi dưới hard floor. Kết thúc cảnh tự nhiên, không thêm độc thoại, giải thích hoặc tổng kết để kéo chữ. title chỉ là tên riêng của chương, không thêm tiền tố "Chương ${input.chapterNumber}:".
PREVIOUS_CHAPTER_TAIL chỉ là điểm nối về hành động và giọng; bắt đầu từ trạng thái sau đoạn đó, không chép hoặc kể lại.
Trước khi trả JSON, tự kiểm tra kín: (1) tổng độ dài ước lượng không dưới ${V3_CHAPTER_LENGTH_POLICY.hardMinWords} từ; nếu còn ngắn, phát triển hành động, đối thoại và phản ứng nhân quả ngay trong các scene đã có, không thêm sự kiện; (2) mọi giá, số lượng, mệnh giá, tiền thối và số dư trong prose thỏa đúng phép tính của WRITER_BRIEF.
...
Giữ chương trong hard range ${V3_CHAPTER_LENGTH_POLICY.hardMinWords}-${V3_CHAPTER_LENGTH_POLICY.hardMaxWords} từ; với full_rewrite hãy nhắm khoảng ${naturalTarget} từ để có biên an toàn trên hard floor. Không thêm đoạn đệm; chỉ phát triển hành động, đối thoại, phản ứng và chuyển tiếp nhân quả bên trong các scene đã khóa.
...
Trước khi trả JSON, tự kiểm tra kín toàn bản sửa: (1) nếu là full_rewrite, tổng độ dài ước lượng không dưới ${V3_CHAPTER_LENGTH_POLICY.hardMinWords} từ; (2) mọi required delta vẫn hiện hữu; (3) mọi giá, số lượng, mệnh giá, tiền thối và số dư khớp phép tính trong ROLE_CONTEXT. Không được chữa một lỗi bằng cách tạo lỗi mới ở ba nhóm này.
