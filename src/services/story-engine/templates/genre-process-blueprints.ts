...
const BLUEPRINTS: Record<GenreType, GenreProcessBlueprint> = {
  ...
  // ════════════════════════════════════════════════════════════════════════
  'vong-du': {
    essence: 'Game/MMO + UI integration + party/guild + PvP/PvE + ranking ladder',
    setup: {
      requiredCastRoles: [
        'Đồng đội cũ (≥1 — pre-existing trust)',
        'Guild master / leader (≥1 — political player)',
        'Top player rival (≥1 — public face-off)',
        'Đối thủ guild lớn (≥1 — institutional)',
        'Game admin / dev (≥1 — meta layer)',
        'Romance / IRL friend',
      ],
      goldenFingerType: 'Hidden skill / kiến thức wiki tương lai / equipment cổ. KHÔNG modify game code — chỉ exploit knowledge.',
      phaseFramework: 'Theo game tier: Solo low-level → Guild mid → Top 100 → Top 10 → Server #1 → Cross-server / e-sport. Mỗi tier ≈ 100-150 ch.',
      worldRulesFocus: [
        'Game mechanics (level, class, skill tree, drop rate, raid spec)',
        'Ranking ladder + reward cycle',
        'Guild politics + alliance war',
        'IRL/in-game blend (life impact of game results)',
      ],
      openingPattern: 'MC login server/level cụ thể. Scene 1: action concrete (solo boss / pvp / quest). Scene 2: result mass-witnessed (server announcement / guild chat blow up). Scene 4-5: top player / guild master tin riêng MC. Hook: opportunity (recruit / raid invite / tournament).',
    },
    process: {
      qualityFloor: 'Mỗi action có damage numbers / time cụ thể / drop names. KHÔNG vague "MC chiến thắng".',
      quantitySustainability: '1000 chương = 5-6 ranking tier × 3-4 raid arc + 2 PvP arc/tier.',
      commonFailures: [
        'IRL bỏ rơi sau giới thiệu → world flat',
        'Game numbers drift (HP/skill names quên)',
        'MC solo mọi thing → guild/team mất ý nghĩa',
      ],
      creativeSpace: 'AI tự sáng tạo: tên guild, tên player, tên item drop. KHÔNG tự sáng tạo: cơ chế game đã established.',
    },
    sceneTypes: [
      'Solo boss / dungeon clear',
      'PvP duel (ranking)',
      'Guild raid (group strategy)',
      'Auction / market (economic)',
      'IRL scene (life impact)',
      'Tournament arena (climax)',
      'Game admin event (meta)',
    ],
    arcTemplate: 'Arc 100-150 ch: ch.1-15 unlock tier mới (zone/level) → ch.16-50 farm + skill build → ch.51-90 guild conflict + PvP escalation → ch.91-120 raid endgame → ch.121-150 server reset / new tier preview.',
    stakesLadder: ['Cá nhân in-game', 'Guild', 'Server top 100', 'Cross-server', 'E-sport quốc gia', 'World championship'],
  },
