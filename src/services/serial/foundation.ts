...
Hợp đồng nền narrativeFoundation là canon tác giả. narrativeEvidence mới là điều độc giả đã đọc. Không cho nhân vật sử dụng fact nếu chưa có trong tri thức ban đầu của họ hoặc chưa có cảnh tạo evidence. Không coi milestone là đã đạt chỉ vì nó có trong hồ sơ.`;
...
Giữ tuyệt đối canon, tài sản, vị trí, thời gian và giới hạn năng lực. Không nhắc prompt hay dữ liệu nội bộ. Trả về chương hoàn chỉnh.`;
  if (role === 'planner') return `Bạn lập kế hoạch chu kỳ cho truyện dài.

${common}

Trả CyclePlan schemaVersion 2. customerLoop có thể null trong giai đoạn chưa kinh doanh; witness và newNamedThing có thể rỗng/null. Lập theo điều kiện đã chuẩn bị thay vì deadline thưởng. Mỗi beat khai prerequisiteIds, revealsFactIds và advancesMilestoneIds bằng stable ID trong nền. Một fact được reveal phải có cảnh tiếp cận thông tin. Chỉ advance milestone sau khi các prerequisite đã có và evidenceNeeded được diễn thành kết quả. Không bắt ba beat đổi sceneMode nếu câu chuyện đang phát triển cùng một việc có biến đổi thực.
...
narrativeEvidence chỉ ghi fact hoặc milestone thực sự được độc giả đọc trong chương. id phải là stable ID có trong narrativeFoundation; quote chép đúng 4-600 ký tự từ prose; learnedByCharacterIds chỉ gồm nhân vật thực sự tiếp cận thông tin. Không ghi plan, suy đoán của Extractor hay kết quả còn đang hẹn.`;
...
export function assertNarrativeDigest(input: {
  ...
  const characters = new Set(bible.symbolicCore.cast.map(item => item.id));
