...
const PLANNER_COMPACT_CONTRACT = {
  ...
  strictRules: [
    ...
    'Mọi chuyển đổi/công suất/quyền hạn/constraint thực sự dùng phải có một mechanics entry tham chiếu worldMechanics ID. Mỗi entry bắt buộc gắn ít nhất một delta bằng primaryDeltaId; các delta còn lại nằm trong additionalDeltaIds. Không tạo mechanics entry nếu cơ chế chưa tạo state transition trong chương. Conversion phải gắn đủ delta đầu vào và đầu ra; tỷ lệ hao hụt đã nằm trong chênh lệch số lượng input/output. Capability phải ghi actor, qty và fact cấp quyền.',
