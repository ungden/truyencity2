...
Trả mechanics trong đúng ba mảng conversions, capabilities và constraints; mỗi mảng có ít nhất một phần tử đúng kind. Conversion chỉ ghi tổng input bị tiêu thụ và output tạo ra theo mỗi batch; tỷ lệ hao hụt nằm trong chênh lệch lượng input/output, còn phụ phẩm cần theo dõi là một output riêng. Capability ghi actor/fact/resource cấp quyền, công suất và chính xác effectResourceIds/effectFactIds mà nó được phép làm thay đổi; ví dụ đánh bắt phải effect resource cá, sửa chữa phải effect trạng thái vật phẩm. Constraint ghi fact bắt buộc hoặc bị cấm. Không giấu số học hoặc quyền hạn trong prose worldRules.
