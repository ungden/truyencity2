...
async function prepareInitProject(
  ...
  if (needsWorldRegen) {
    try {
      ...
      const prompt = `Bạn là tác giả webnovel chuyên nghiệp. ${needsMcRegen ? 'Tạo MỚI HOÀN TOÀN' : 'Tạo lại'} setup cho tiểu thuyết.
...
${needsMcRegen ? '- mainCharacter PHẢI đa dạng — không dùng tên cliché phổ biến (Lâm Phong, Lê Minh, Trần Vũ, Trần Hạo, Lý Phong). Tùy theo genre + bối cảnh, chọn họ + tên hợp lý.' : ''}`;
      ...
      if (newWorldDesc && newWorldDesc.length >= 500) updates.world_description = newWorldDesc;
      if (needsMcRegen && newMc && newMc.length >= 2) updates.main_character = newMc;

      if (Object.keys(updates).length > 0) {
        await supabase.from('ai_story_projects').update(updates).eq('id', project.id);
        if (updates.world_description) projRow = { ...projRow, world_description: newWorldDesc, story_outline: projRow?.story_outline ?? null, story_bible: projRow?.story_bible ?? null, master_outline: projRow?.master_outline ?? null };
      }
    } catch (e) {
      console.warn(`[init-prep] world_description+mc regen failed for ${project.id}:`, e instanceof Error ? e.message : String(e));
    }
  }
