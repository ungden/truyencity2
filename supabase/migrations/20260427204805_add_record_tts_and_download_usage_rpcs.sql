...
-- record_tts_usage(p_user_id, p_seconds): increments today's TTS seconds for user,
-- returns new total + daily limit + can_continue flag. Mobile reader calls this
-- every 60s while playing; once can_continue=false the client stops + paywall.
...
BEGIN
  ...
  -- Determine effective tier (downgrade expired VIP).
  ...
  -- Upsert today's row, accumulating seconds.
...
-- record_download_usage(p_user_id, p_chapters): increments today's download
-- counter, returns new total + daily limit + can_continue flag. Library
-- batch-download calls this before queuing the chapter fetch.
