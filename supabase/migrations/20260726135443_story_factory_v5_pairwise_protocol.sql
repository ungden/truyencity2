...
SET benchmark_protocol_version = 'legacy_incomparable',
    output_artifact = COALESCE(output_artifact, '{}'::jsonb) || jsonb_build_object(
      'supersededBy', 'story-factory-validation-v5-pairwise-sequential-reader',
      'supersededAt', now()
    )
WHERE kind = 'benchmark'
  AND benchmark_protocol_version IN (
    'story-factory-validation-v4-causal-reader',
    'story-factory-writer-bakeoff-v3-causal-qualified',
    'story-factory-sequential-survival-v2-causal-continuity'
  );
...
AS $$
  SELECT EXISTS (
    ...
    WHERE benchmark.id = p_benchmark_id
      ...
      AND writer_validation.benchmark_protocol_version = 'story-factory-writer-bakeoff-v4-causal-discovery'
