...
async function main(): Promise<void> {
  ...
  console.log(JSON.stringify({ projectId, status: result.artifact.status, finalists: result.artifact.ranking.finalistIds, rejectedNearDuplicates: result.artifact.rejectedNearDuplicateIds, calls: result.callRoles, nextGate: 'human_select_one_finalist' }, null, 2));
}
