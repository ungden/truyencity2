 ...
 * factory injects into the Planner payload and the Writer brief of a RUNNING
 * novel (ai_story_projects.author_directive). Applies from the next chapter the
 * cron writes; already-published chapters are untouched. Canon and the causal
 * validator always outrank the directive.
 *
 * Usage:
...
async function main() {
  ...
  if (command === 'set' || command === 'clear') {
    ...
    console.log(text
      ? `Directive set for ${titleOf(project)} — applies from the next chapter:\n${text}`
